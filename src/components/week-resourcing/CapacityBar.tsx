...
export const CapacityBar: React.FC<CapacityBarProps> = ({
  ...
  return <div className="flex items-center justify-center w-full">
      <div className="flex-1 flex justify-center items-center gap-1.5">
        ...
        <div className="flex gap-[0.5px]">
          ...
          return <Tooltip key={index}>
                <TooltipTrigger asChild>
                  <div className={cn("h-2.5 w-2.5 relative overflow-hidden", !isFilled && !isPartiallyFilled && "bg-gray-100", isFilled || isPartiallyFilled ? "" : "border border-gray-300")}>
                    {/* For completely filled boxes */}
                    {isFilled && <div className={cn("absolute inset-0", boxColor)} />}
                    
                    {/* For partially filled boxes */}
                    {isPartiallyFilled && <div className={cn("absolute top-0 bottom-0 left-0", boxColor)} style={{
                ...
                <TooltipContent>
                  <p className="text-xs">
                    {isFilled ? 'Used (100%)' : isPartiallyFilled ? `Used (${Math.min(100, Math.max(0, (percentageUsed - boxStartPercent) / 20 * 100)).toFixed(0)}%)` : 'Available'}
                  </p>
                </TooltipContent>
              </Tooltip>;
        })}
        ...
        {/* Available hours number - directly without additional container */}

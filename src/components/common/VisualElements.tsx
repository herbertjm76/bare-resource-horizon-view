...
export const FloatingIcons = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    <div className="absolute top-20 left-10 w-8 h-8 text-purple-300/20 animate-[float_6s_ease-in-out_infinite] hover:text-purple-300/40 transition-colors duration-300">
...
export const VisualCard = ({ 
  ...
  <div 
    className={`
      ...
      ${hoverEffect ? 'transition-all duration-700 hover:shadow-2xl hover:-translate-y-2 hover:scale-105 hover:border-purple-200 cursor-pointer group' : ''}
      ${className}
    `}
    style={{
      ...style,
  ...
  >
    {hoverEffect && (
      <>
        ...
        <div className="absolute top-2 right-2 w-2 h-2 bg-purple-500 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-0 group-hover:scale-100"></div>
      </>
    )}
    {children}
  </div>
...
export const GradientOrbs = () => (
  <div className="absolute inset-0 overflow-hidden pointer-events-none">
    <div className="absolute -top-20 -left-20 w-96 h-96 bg-gradient-to-br from-purple-400/20 to-blue-400/20 rounded-full blur-3xl animate-[float_15s_ease-in-out_infinite] hover:scale-110 transition-transform duration-700"></div>
  ...
  <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
    {stats.map((stat, index) => (
      <div key={index} className="text-center group cursor-pointer">
        <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-blue-500 rounded-2xl flex items-center justify-center mx-auto mb-3 transition-all duration-700 group-hover:scale-125 group-hover:rotate-12 group-hover:shadow-xl group-hover:shadow-purple-500/25">
          <stat.icon className="w-8 h-8 text-white transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-12" />
        </div>
        <div className="text-2xl font-bold text-gray-900 mb-1 transition-all duration-300 group-hover:text-purple-600 group-hover:scale-110">{stat.value}</div>

        ...
        {financialMetrics && (
          <Card>
            ...
            <CardContent>
              <ProgressIndicator
                label="Budget Spent"
                current={financialMetrics.total_spent || 0}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={Math.abs(financialMetrics.budget_variance || 0) > 1000 ? 'destructive' : 'default'}>
                    ${(financialMetrics.budget_variance || 0).toLocaleString()}
                  </Badge>
                </div>
                <div className="flex items-center gap-2">
                  {(financialMetrics.budget_variance || 0) > 0 ? (
                    <TrendingUp className="h-4 w-4 text-red-500" />
                  ) : (
                    <TrendingDown className="h-4 w-4 text-green-500" />
                  )}
                  <span className={`text-sm ${getVarianceColor(financialMetrics.budget_variance || 0)}`}>
                    {(financialMetrics.budget_variance || 0) > 0 ? 'Over budget' : 'Under budget'}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-sm text-muted-foreground">
                    Profit Margin: {(financialMetrics.profit_margin || 0).toFixed(1)}%
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t">
              <div className="text-center">
                <div className="text-lg font-semibold">
                  {(financialMetrics.consumed_hours || 0).toFixed(1)}
                </div>
                <div className="text-xs text-muted-foreground">Hours Consumed</div>
              </div>
              <div className="text-center">
                <div className="text-lg font-semibold">
                  {(financialMetrics.budget_hours || 0).toFixed(1)}
                </div>
